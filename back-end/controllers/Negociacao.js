...
controller.novo = async (req, res) => {
   try {
      await Negociacao.create(req.body)
...
controller.obterUm = async (req, res) => {
   ...
   try {
      ...
      const obj = await Negociacao.findById(id)
...
controller.atualizar = async (req, res) => {
   try {
      ...
      const obj = await Negociacao.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
   try {
      const id = req.body._id
      const obj = await Negociacao.findByIdAndDelete(id)
      if (obj) {
         res.status(204).end()
      }
      else {
...
async function busca(req, res) {
   ...
   try {
      const lista = await Negociacao.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)
