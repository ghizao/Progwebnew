...
controller.obterUm = async (req, res) => {
   ...
   try {
      ...
      const obj = await Profissional.findById(id)
...
controller.atualizar = async (req, res) => {
   try {
      ...
      const obj = await Profissional.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
   try {
      const id = req.body._id
      const obj = await Profissional.findByIdAndDelete(id)
      if (obj) {
         res.status(204).end()
      }
      else {
...
async function busca(req, res) {
   ...
   try {
      const lista = await Profissional.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)
